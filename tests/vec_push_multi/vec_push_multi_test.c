...
int vec_push_multi_test() {
    ...
    vec_push_multi(&vec, (void*)&INSERT_DATA1, INSERT_DATA1_LEN);

    bool contains_all = true;
    // Check for the initial data
    for (int i = 0; i < DATA2_LEN; i++) {
        if (*(int*)vec_get_s(&vec, i) != DATA2[i]) { contains_all = false; }
    }
    // Check for additional data
    for (int i = DATA2_LEN; i < DATA2_LEN + INSERT_DATA1_LEN; i++) {
        if (*(int*)vec_get_s(&vec, i) != INSERT_DATA1[i - DATA2_LEN]) { contains_all = false; }
    }
    ...
    for (int i = DATA1_LEN; i < DATA1_LEN + INSERT_DATA1_LEN; i++) {
        if (*(int*)vec_get_s(&vec, i) != INSERT_DATA1[i - DATA1_LEN]) { contains_all = false; }
    }
