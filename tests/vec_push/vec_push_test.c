...
int vec_push_test() {
    ...
    end_test(
        vec.len == DATA2_LEN+1 && vec.cap == DATA2_CAP && *(int*)vec_get_unchecked(&vec, DATA2_LEN) == INSERT_DATA1[0],
        vec_drop_single(&vec)
    );
    ...
    end_test(
        vec.len == DATA1_LEN+2 && vec.cap == 16 && *(int*)vec_get_unchecked(&vec, DATA1_LEN) == INSERT_DATA1[0] && *(int*)vec_get_unchecked(&vec, DATA1_LEN+1) == INSERT_DATA1[1],
        vec_drop_single(&vec)
    );
