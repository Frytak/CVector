...
int vec_remove_test() {
    ...
    start_test(2);
    vec = vec_new(DATA2_SIZE, (void*)DATA2, DATA2_LEN);
    vec_remove_unchecked(&vec, vec.len-1);
    ...
    start_test(3);
    vec = vec_new(DATA2_SIZE, (void*)DATA2, DATA2_LEN);
    vec_remove_unchecked(&vec, 3);

    end_test(
        vec.len == DATA2_LEN-1 && vec.cap == DATA2_CAP && vec.size == DATA2_SIZE && vec.data != NULL
        && (memcmp(vec_get_unchecked(&vec, 0), DATA2, vec.size*3) == 0)
        && (memcmp(vec_get_unchecked(&vec, 3), ((void*)DATA2+(vec.size*4)), (vec.len-3) * vec.size) == 0),
        vec_drop_single(&vec)
    );
    ...
    start_test(4);
    vec = vec_new(DATA2_SIZE, (void*)DATA2, DATA2_LEN);
    VEC_REMOVE_RESULT error1 = vec_remove(NULL, 0);
    VEC_REMOVE_RESULT error2 = vec_remove(&vec, vec.len+4);
    VEC_REMOVE_RESULT ok = vec_remove(&vec, 2);
    drop_test_vecs(vec_drop_single(&vec));
    VEC_REMOVE_RESULT error3 = vec_remove(&vec, 0);
