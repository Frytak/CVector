...
int vec_binary_search_test() {
    ...
    start_test(11);
    searched_index = 1;
    VEC_RESULT error1 = vec_binary_search(&vec, vbsc_int, 2, vec.len, &index, (void*)&DATA4[searched_index]);
    VEC_RESULT error2 = vec_binary_search(NULL, vbsc_int, 0, vec.len, &index, (void*)&DATA4[searched_index]);
    VEC_RESULT error3 = vec_binary_search(&vec, NULL, 0, vec.len, &index, (void*)&DATA4[searched_index]);
    VEC_RESULT error4 = vec_binary_search(&vec, vbsc_int, 0, vec.len, &index, NULL);
    VEC_RESULT error5 = vec_binary_search(&vec, vbsc_int, 128, 234, &index, (void*)&DATA4[searched_index]);
    VEC_RESULT error6 = vec_binary_search(&vec, vbsc_int, vec.len, vec.len+1, &index, (void*)&DATA4[searched_index]);
    VEC_RESULT error7 = vec_binary_search(&vec, vbsc_int, vec.len, 2, &index, (void*)&DATA4[searched_index]);
    VEC_RESULT error8 = vec_binary_search(&vec, vbsc_int, vec.len, vec.len, &index, (void*)&DATA4[searched_index]);
    VEC_RESULT ok = vec_binary_search(&vec, vbsc_int, 0, vec.len, &index, (void*)&DATA4[searched_index]);
